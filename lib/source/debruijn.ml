(* Scope resolution: named surface AST -> de Bruijn indexed surface AST.
  ...
  - EVar "x" (named variables) are transformed into a `path`.
  - `TVar "a"` becomes a de Bruijn index
  - All type aliases are expanded.

  Frames are used for pass to enable consistency when keeping map from 
  named variables to de Bruijn index.
*)
open Ast
...
let rec shape_of_typ (t : 'tv typ) : shape =
  match t.it with
  | TRcd fs     -> 
      (* Creates a list of (label, shape for type) and merges *)
      SRcd (List.map (fun (l, ft) -> (l, shape_of_typ ft)) fs)
  | TAnd (a, b) -> 
      merge_shape (shape_of_typ a) (shape_of_typ b)
  | TOr (a, b)  -> 
      SOr (shape_of_typ a, shape_of_typ b)
  | TArr (_, b) | TSig (_, b)   -> 
      SFun (shape_of_typ b)
  | TInt | TBool | TString | TTop | TVar _ | TMu _ -> 
      SOpaque

(* Accessors. A shape we cannot see into has no fields and no result, so every
   one of these answers SOpaque (or no fields) rather than failing: the name
   still resolves to a path, and Sugar rejects it later if there is no such
   field. A slot we cannot see into still occupies an index. *)

let fun_result      = function SFun s -> s    | _ -> SOpaque
let fields_of       = function SRcd fs -> fs  | _ -> []
let field_shape s l = Option.value ~default:SOpaque (List.assoc_opt l (fields_of s))
...
module F = Frames.Make (struct type t = frame end)
...
(* Each label carries the location to blame if it turns out to be a repeat. *)
let rec check_dups what seen = function
  | [] -> ()
  | (loc, l) :: rest ->
    if List.mem l seen then duplicate loc what l
    else check_dups what (l :: seen) rest

(* Every label is blamed on the same node — the record or record type itself. *)
let check_no_duplicates loc what labels =
  check_dups what [] (List.map (fun l -> (loc, l)) labels)

(* Reported at the offending declaration rather than at the head of the list. *)
let check_no_duplicate_decls what ds =
  check_dups what []
    (List.filter_map (fun d -> Option.map (fun l -> (d.loc, l)) (name_of_decl d)) ds)
