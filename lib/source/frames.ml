(* The λSCE context discipline, in one place.
   ...
   binder extends it. Two passes have to agree on exactly how — Debruijn, which
   turns names into indices into it, and Sugar, which rebuilds it as a type. If
   they ever disagree the indices silently point at the wrong slot, so both
   instantiate this module rather than open-coding the pushes.
