...
(* Imported functions occupy the front of the function index space, so every
   reference to a local function is offset by the import count. `CallImport`
   is the only instruction that addresses the import range directly. *)
let rec instr ~nimp buf = function
  ...
  | Call i -> byte buf 0x10; u32 buf (nimp + i)
  | CallImport i -> byte buf 0x10; u32 buf i
  | CallRef t -> byte buf 0x14; u32 buf t
  | RefFunc f -> byte buf 0xd2; u32 buf (nimp + f)
  ...
  | If (bt, thn, els) ->
    byte buf 0x04;
    blocktype buf bt;
    List.iter (instr ~nimp buf) thn;
    if els <> [] then begin
      byte buf 0x05;
      List.iter (instr ~nimp buf) els
    end;
    byte buf 0x0b
  | Block (bt, body) ->
    byte buf 0x02; blocktype buf bt; List.iter (instr ~nimp buf) body; byte buf 0x0b
  | Loop (bt, body) ->
...
let code ~nimp buf f =
  ...
  List.iter (instr ~nimp body) f.fn_body;
  byte body 0x0b;
  u32 buf (Buffer.length body);
  Buffer.add_buffer buf body

let global ~nimp buf g =
  valtype buf g.gl_type;
  byte buf (if g.gl_mut then 0x01 else 0x00);
  List.iter (instr ~nimp buf) g.gl_init;
  byte buf 0x0b

let export ~nimp buf e =
  name buf e.ex_name;
  byte buf 0x00;
  u32 buf (nimp + e.ex_func)

let modul (m : modul) : string =
  let nimp = List.length m.m_imports in
  ...
  section buf 6 (fun b -> vec b (global ~nimp) m.m_globals);
  section buf 7 (fun b -> vec b (export ~nimp) m.m_exports);
  ...
  if m.m_declared <> [] then
    section buf 9 (fun b ->
        ...
        vec b (fun b i -> u32 b (nimp + i)) m.m_declared);
  ...
  section buf 10 (fun b -> vec b (code ~nimp) m.m_funcs);
  if m.m_datas <> [] then
