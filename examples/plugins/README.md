...
them, and links each against an attenuated capability record — with the
language's own `link`, at the use site:
`(link { Cap = ... } with p).run name`. The wire is label-preserving rather
than renaming, so the host aligns labels by building the capability record
under the label the plugin imports; first-class linking then applies to a
functor that arrived from disk exactly as it does to one written in place.
